...
const Navbar = () => {
  ...
  return (
    <div className="sticky !w-[95dvw] mx-auto  top-4 -mt-[4.5rem] z-50 transition-all duration-300">
      ...
      > */}
        <div className="container-tight flex items-center justify-between px-4 w-full">
          <a
            href="#hero"
            className={
              "text-lg [text-shadow:_0px_0px_1px_rgba(0,0,0,1)] font-bold tracking-tight transition-colors duration-[5000ms] " +
              ...
              <a
                ...
                className={`text-sm transition-all duration-200 relative cursor-target px-3 py-1 ${
                  activeSection === l.href.slice(1)
                    ? "text-green-600"
                    : (scrolled ? "text-white" : "text-white") +
                      " hover:text-foreground [text-shadow:_0px_0px_1px_rgba(0,0,0,1)]"
                }`}
              >
                {l.label}
                {activeSection === l.href.slice(1) && (
      ...
      {open && (
        <div className="md:hidden absolute w-full mt-2">
          <GlassSurface
            displace={0.5}
            distortionScale={-180}
            redOffset={0}
